...
export function WorkspaceSidebar({
    ...
    const jupyterFiles = React.useMemo(() =>
        filteredFiles.filter(f => f.language === 'jupyter'),
        [filteredFiles]
    )

    const otherFiles = React.useMemo(() =>
        filteredFiles.filter(f => !['python', 'sql', 'jupyter'].includes(f.language)),
        [filteredFiles]
    )
    ...
    const renderFileItem = (file: ProjectFile) => (
        <SidebarMenuItem key={file.id}>
            ...
            >
                {file.language === 'python' && <FileCode className="size-4 opacity-70" />}
                {file.language === 'sql' && <Database className="size-4 opacity-70" />}
                {file.language === 'jupyter' && <BookOpen className="size-4 opacity-70" />}
                {file.language === 'markdown' && <FileText className="size-4 opacity-70" />}
                {file.language === 'json' && <FileJson className="size-4 opacity-70" />}
        ...
        <Sidebar collapsible="icon" {...props} className="border-r border-border/40 bg-sidebar/80 backdrop-blur-xl">
            ...
            <SidebarContent>
                ...
                <SidebarGroup>
                    ...
                    <SidebarGroupContent>
                        <SidebarMenu>
                            {jupyterFiles.map(renderFileItem)}
                        </SidebarMenu>
                    </SidebarGroupContent>
                </SidebarGroup>
                ...
                {otherFiles.length > 0 && (
                    <>
                        ...
                        <SidebarGroup>
                            <div className="flex items-center justify-between px-2 py-1">
                                <div className="flex items-center gap-2">
                                    <FileIcon className="size-3.5 text-gray-500" />
                ...
                <SidebarGroup className="mt-auto border-t border-border/20 pt-4 pb-2 group-data-[collapsible=icon]:hidden">
                    <div className="px-3">
                        ...
                        <div className="flex gap-6">
                            ...
                            <div className="flex flex-col gap-0.5">
                                <span className="text-[10px] text-muted-foreground/60 uppercase font-medium">Notebooks</span>
                                <span className="text-base font-bold text-orange-500/90">{jupyterFiles.length}</span>
                            </div>
                        </div>
                    </div>
                </SidebarGroup>
